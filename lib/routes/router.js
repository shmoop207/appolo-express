...
class Router {
    ...
    addRoutes(id, routes) {
        var abstract =  _(routes).remove(route=> route.abstract).first();
    ...
    _createRoute(route) {
        ...
        var method = route.method || "get",
            middleware = _.flatten((route.middleware || []),(route.middlewares || []));
        ...
        var args = _.map(middleware, middlewareId => _.isFunction(middlewareId)
            ? middlewareId
    ...
    _onValidationSuccess(route, req, res, next, params) {
        ...
        if (route.validations.convertToCamelCase) {
            ...
            _.forEach(params, (value, key) => {
                output[_.convertSnakeCaseToCamelCase(key)] = value
            });
        }
        else {
            output = params;
