...
module.exports = Class.define({
    ...
    json:function(statusCode,data) {
        ...
        if (arguments.length === 1) {
            this.json200(arguments[0])
        } else {
            this.res.status(statusCode).jsonp(data);
        }
    },

    json200:function(data) {
        this.res.status(200).jsonp(data);
    },

    json201:function(data) {
        this.res.status(201).jsonp(data);
    },
    json204:function(data) {
        this.res.status(204).jsonp(data);
    },

    json500:function(error) {
